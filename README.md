...
The TooltipTrigger class can serve as a simple example of API usage.
...
However, you may want to inject it through DI. It wasn't included in the example to keep the code simple. Tooltip creation happens in the TooltipsFactory class, with simple pooling, so it will be easy to replace it with your DI Container's factory implementation.
...
You will also likely need to localize it, which will be easy to do, for example, in the TooltipSystem class.
...
- `Preparing`: the cursor is over the item, but the tooltip is waiting for a delay to be shown (`Show Delay`).
- `Shown`: the tooltip is shown, waiting for a delay to be fixed (`Make Fixed Delay`).
...
The `Tooltip Prefab` is made as simple as possible. You can redesign it however you like, just make sure to keep the set of components on the root GameObject. Adding Tooltip animations should also be easy, as the code is very simple.

Art for the UI is taken from this asset: [Clean Settings UI](https://assetstore.unity.com/packages/tools/gui/clean-settings-ui-65588).
